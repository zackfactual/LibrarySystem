...
CREATE PROC dbo.uspGetSharpstownLostTribeCopiesALT AS
SELECT SUM(copies_number) AS 'Sharpstown copies of "The Lost Tribe"'
FROM (tbl_copies copies INNER JOIN tbl_book book on copies.copies_bookid = book.book_id)
INNER JOIN tbl_branch branch on branch.branch_id = copies.copies_branchid
WHERE book.book_title = 'The Lost Tribe' AND branch.branch_name = 'Sharpstown';
GO

--EXEC dbo.uspGetSharpstownLostTribeCopiesALT allows you to just change title and branch name, it's more versatile
...
CREATE PROC dbo.uspGetLostTribeCopiesPerBranchALT AS
SELECT SUM(copies_number) AS 'Copies of "The Lost Tribe"', branch.branch_name
FROM (tbl_copies copies INNER JOIN tbl_book book on copies.copies_bookid = book.book_id)
INNER JOIN tbl_branch branch on branch.branch_id = copies.copies_branchid
WHERE book.book_title = 'The Lost Tribe' GROUP BY branch.branch_name;
GO

--EXEC dbo.uspGetLostTribeCopiesPerBranchALT
...
CREATE PROC dbo.uspCentralGetKingTitleCopiesALT AS
SELECT a1.book_title AS 'Stephen King Title', a2.copies_number AS '# of Copies at Central'
FROM tbl_book a1
INNER JOIN tbl_copies a2 ON a2.copies_bookid = a1.book_id
INNER JOIN tbl_branch a3 ON a3.branch_id = a2.copies_branchid
INNER JOIN tbl_author a4 ON a4.author_id = a1.book_authorid
WHERE a3.branch_name = 'Central' AND a4.author_name = 'Stephen King';
GO

--EXEC dbo.uspCentralGetKingTitleCopiesALT